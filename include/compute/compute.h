...
//哈希值：就是一个无序唯一值，可以理解每个Vertex数据生成一个类似身份证号的标识；
//定义哈希函数，为了让自定义结构体 Vertex 能够作为 std::unordered_map 的键（Key），进行顶点去重
namespace std {
...
private:
    ...
    */
    ...
    //信号量--用于GPU
    std::vector<VkSemaphore>_imageAvailableSemaphores;//已从交换链获取图像并准备好渲染，等待交换链呈现完毕信号量
    std::vector <VkSemaphore> _renderFinishedSemaphores;//渲染绘制已完成并可以进行呈现信号量
    //栅栏--用于CPU
    ...
    std::vector <VkSemaphore> _computeFinishedSemaphores;//渲染绘制已完成并可以进行呈现信号量
    //阻塞CPU，等待GPU
